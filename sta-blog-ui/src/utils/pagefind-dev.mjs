/**
 * 开发脚本：用 Pagefind JS API 为所有文章生成离线搜索索引
 * 输出到 src/pagefind/，开发时由 Vite 模块系统直接加载
 */
...
const OUTPUT_PATH = join(PROJECT_ROOT, "src/pagefind");
...
async function main() {
  console.log("[Pagefind Dev] 读取文章清单...");
  const raw = readFileSync(ARTICLES_JSON, "utf-8");
  const articles = JSON.parse(raw);
  console.log(`[Pagefind Dev] 共 ${articles.length} 篇文章`);
  ...
  await close();
  console.log(`[Pagefind Dev] ✅ 索引已生成到 ${OUTPUT_PATH}（${indexed} 篇）`);
}

main().catch((err) => {
  console.error("[Pagefind Dev] ❌ 失败:", err);
  process.exit(1);
});
