...
/** /localSync/article-content 返回条目 */
...
function saveEncrypted(filename: string, data: any): void {
    const newFilename = isCrypto() ? filename.replace(/\.json$/, '.enc') : filename
    fs.writeFileSync(newFilename, encrypt(data))
    console.log(`  ✅ ${path.basename(newFilename)} 已生成`)
}

/** 保存文章内容为 .md 文件（不加密） */
function saveMarkdown(filePath: string, content: string): void {
    fs.mkdirSync(path.dirname(filePath), { recursive: true })
    fs.writeFileSync(filePath, content, 'utf-8')
    console.log(`  ✅ ${path.basename(filePath)} 已生成`)
}
...
async function fetchWebsiteInfo(): Promise<WebsiteInfo | null> {
    try {
        const res = await fetchWithRetry<ApiResponse<WebsiteInfo>>(`${API_BASE_URL}/websiteInfo/front`)
...
/** 获取所有文章内容 → 保存为 articles/${id}.md */
async function fetchArticleContents(): Promise<void> {
    try {
        const res = await fetchWithRetry<ApiResponse<ArticleContentVO[]>>(`${API_BASE_URL}/localSync/article-content`)
        ...
        for (const item of list) {
            const mdPath = path.join(ARTICLES_DIR, `${item.id}.md`)
            saveMarkdown(mdPath, item.articleContent || '')
        }
    } catch (error: any) {
...
async function fetchArticleDetails(): Promise<ArticleDetailVO[]> {
    try {
        const res = await fetchWithRetry<ApiResponse<ArticleDetailVO[]>>(`${API_BASE_URL}/localSync/article-detail`)
        if (res.code !== 200) throw new Error(res.msg)
...
async function fetchCategoryWithArticle(): Promise<void> {
    try {
        const res = await fetchWithRetry<ApiResponse<CategoryWithArticleVO[]>>(`${API_BASE_URL}/localSync/category-with-article`)
        if (res.code !== 200) throw new Error(res.msg)
...
async function fetchTagWithArticle(): Promise<void> {
    try {
        const res = await fetchWithRetry<ApiResponse<TagWithArticleVO[]>>(`${API_BASE_URL}/localSync/tag-with-article`)
        if (res.code !== 200) throw new Error(res.msg)
...
async function main(): Promise<void> {
    ...
    // 从 article-detail 中提取 article-list（去掉 content 减小体积）
    if (articleDetails.length > 0) {
        const articleList = articleDetails.map(({ articleContent, ...rest }) => rest)
        saveEncrypted(path.join(API_DIR, 'article-list.json'), articleList)
        console.log(`  ✅ article-list.json 已生成（${articleList.length} 条，不含内容）`)

        // 提取搜索标题
        const searchTitles = articleDetails.map(({ id, articleTitle }) => ({ id, articleTitle }))
        saveEncrypted(path.join(API_DIR, 'search-titles.json'), searchTitles)
        console.log(`  ✅ search-titles.json 已生成（${searchTitles.length} 条）`)
    }
