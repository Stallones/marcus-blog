...
function isLikeFunc() {
  isLike(3, leaveWord.value.id).then((res: any) => {
...
function userLikeFunc() {
  ...
  userLike(3, leaveWord.value.id).then((res: any) => {
    if (res.code === 200) {
...
function cancelLikeFunc() {
  ...
  cancelLike(3, leaveWord.value.id).then((res: any) => {
    if (res.code === 200) {
      leaveWord.value.likeCount--
      like.value = false
    } else {
...
function isFavoriteFunc() {
  isFavorite(2, leaveWord.value.id).then((res: any) => {
...
function userFavoriteFunc() {
  ...
  userFavorite(2, leaveWord.value.id).then((res: any) => {
    if (res.code === 200) {
...
function cancelFavoriteFunc() {
  ...
  cancelFavorite(2, leaveWord.value.id).then((res: any) => {
    if (res.code === 200) {
      leaveWord.value.favoriteCount--
      favorite.value = false
    } else {
